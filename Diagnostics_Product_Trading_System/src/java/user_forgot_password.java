        ...
        if(event.equals("submit"))
         {
              if(email.equals("")||password.equals("") || contact.equals(""))
             {
                     ...
                     out.println("location='user_frogot_password.jsp';");
                     out.println("</script>");
             }
             else
             {
                try
                {
                     String update = db.update("update user_signup set  password='"+password+"', cpassword='"+password+"' where contact_no='"+contact+"' && email='"+email+"' ");
                     out.println(update);
                     resp.setContentType("text/html");
                     out.println("<script type=\"text/javascript\">");
